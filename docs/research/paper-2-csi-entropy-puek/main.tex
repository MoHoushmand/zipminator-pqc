...
We present the first system, measurement, and preliminary NIST SP~800-90B assessment of WiFi CSI as a \emph{unilateral} entropy source: a single device passively measuring ambient CSI to harvest genuine physical randomness with no cooperating partner.
The assessment is pilot-scale (2{,}690~bytes), below the $\geq$1~M-sample threshold that NIST recommends for a production \texttt{ea\_non\_iid} certification; results should be read as a methodological first step, not a production entropy certificate.
...
The method is hardware-agnostic, working with any IEEE~802.11n/ac/ax device; a \$5 ESP32-S3 reference implementation is projected to produce 45--90~MB/month at zero marginal cost (extrapolated from our BCM4339 Nexmon baseline), compared to IBM~Quantum at \$1.60/second.
...
\item \textbf{First preliminary NIST SP 800-90B assessment of WiFi CSI.}
Using the public Gi-z/CSI-Data corpus~\cite{gringoli2019csidata}, we run the full \texttt{ea\_non\_iid} assessment and report 5.50~bits/byte final min-entropy from 343~Nexmon frames (Section~\ref{sec:evaluation}); the 2{,}690-byte sample is a pilot-scale demonstration below the $\geq$1~M-sample NIST recommendation for production certification.
...
\subsection{Throughput (projected)}
\label{subsec:throughput}

The following ESP32-S3 figures are \emph{projections} extrapolated from our BCM4339 Nexmon baseline; no ESP32-S3 capture was run in this pilot. An ESP32-S3 receiving WiFi frames at 20~Hz with 256 subcarriers is projected to generate:
\begin{equation}
\text{Raw rate} = 20 \times 256 = 5{,}120 \; \text{bits/s}
\label{eq:raw-rate}
\end{equation}
After Von~Neumann debiasing at 24.5\% (the ratio measured on the Nexmon trace):
\begin{equation}
\text{Debiased rate} = 5{,}120 \times 0.245 = 1{,}254 \; \text{bits/s} \approx 157 \; \text{bytes/s}
\label{eq:debiased-rate}
\end{equation}
This projects to approximately \textbf{13.5~KB/hour} or \textbf{9.7~MB/month} per node.
A mesh of 5 nodes projects to 48.5~MB/month, and with 10 nodes, 97~MB/month.
At higher frame rates (100~Hz on Nexmon-patched hardware), throughput scales linearly to 785~bytes/s or 45~MB/month per node (projected).
...
\begin{table}[t]
\centering
\caption{Cost comparison of entropy sources. ESP32-S3 CSI throughput is a projection extrapolated from our BCM4339 Nexmon baseline; no ESP32-S3 capture was run.}
\label{tab:economics}
\small
\begin{tabular}{@{}lccc@{}}
\toprule
...
Entropy/month (free) & $\sim$6~MB\footnotemark & 45--90~MB (projected) & Unlimited \\
...
At a projected 45--90~MB/month (depending on frame rate, extrapolated from BCM4339 Nexmon measurements), the amortized cost per MB drops below \$0.001 within the first month.
...
An entropy system that explicitly pauses during low-quality conditions is strictly preferable to one that silently outputs low-entropy bytes; the variance monitor is a feature, not a limitation.
...
In particular, Mathur~\etal~\cite{mathur2008radio} explicitly require a cooperating partner and a reciprocal channel measurement, because their construction reduces key agreement to matching quantized bits between Alice and Bob; without the second endpoint, their scheme yields no output.
Our construction drops both the cooperating-partner and reciprocity assumptions: a single device passively measures ambient CSI and extracts min-entropy without a counterparty.
...
Neighboring OFDM subcarriers experience similar channel conditions (coherence bandwidth $\approx$ 2--5~MHz in indoor environments~\cite{halperin2011tool}, spanning 10--25 subcarriers at 20~MHz).
...
\item \textbf{Pilot-scale measured min-entropy.}
...
This is the first preliminary SP~800-90B assessment of WiFi CSI in any configuration; the 2{,}690-byte sample is below the $\geq$1~M-sample recommendation for a production certification, and a full-scale assessment remains as future work.
...
\item \textbf{Economic viability (projected).}
A \$5 ESP32-S3 is projected to produce 45--90~MB/month of physical entropy at zero marginal cost (extrapolated from our BCM4339 Nexmon baseline; no ESP32-S3 capture was run), compared to \$1.60/second for cloud quantum entropy.
For IoT deployments requiring hardware-backed TRNG, CSI entropy is projected to offer a 4--6 order-of-magnitude cost reduction over QRNG.
